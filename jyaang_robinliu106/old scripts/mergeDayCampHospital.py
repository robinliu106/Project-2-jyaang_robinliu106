...
class mergeDayCampHospital(dml.Algorithm):
    ...
    reads = ['alice_bob.dayCamp','alice_bob.dayCamp']
    writes = ['alice_bob.mergeHospital_dayCamp']
    ...
    def execute(trial = False):
        ...
        repo.alice_bob.mergeDayCampHospital.drop()

        for record in repo.alice_bob.hospital.find():
            repo.alice_bob.mergeDayCampHospital.insert(record,check_keys=False)

        for record in repo.alice_bob.dayCamp.find():
            repo.alice_bob.mergeDayCampHospital.insert(record,check_keys=False)

        repo.alice_bob.mergeDayCampHospital.map_reduce(cityMap,r,'repo.alice_bob.mergeDayCampHospital')
    ...
    def provenance(doc = prov.model.ProvDocument(), startTime = None, endTime = None):
        ...
        this_script = doc.agent('alg:alice_bob#mergeDayCampHospital', {prov.model.PROV_TYPE:prov.model.PROV['SoftwareAgent'], 'ont:Extension':'py'})
        ...
        mergeDayCampHospital = doc.entity('dat:alice_bob#mergeDayCampHospital', {prov.model.PROV_LABEL:'merge Day Camp Hospital', prov.model.PROV_TYPE:'ont:DataSet'})
